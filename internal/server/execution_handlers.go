...
func (s *Server) handleCreateExecution(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve CWD — for executing phase, create a worktree first
	cwd := resolveCWD(req.Phase, req.IssueID, eng)

	if req.Phase == "executing" && req.Params["issue"] != "" {
		issueID, issueTitle := parseIssueParam(req.Params["issue"])
		repoPath := "."
		if eng != nil && len(eng.State.Repos) > 0 {
			// Find repo from issue state, fall back to first repo
			...
			if repoPath == "." {
				repoPath = eng.State.Repos[0].Path
			}
		}

		slug := serverSlugify(issueID + "-" + issueTitle)
		wm := gitops.NewWorktreeManager(".")
		wtPath, err := wm.Create(repoPath, slug)
		if err != nil {
			log.Printf("[exec] %s: failed to create worktree: %v", executionID, err)
			// Continue with resolved CWD as fallback
		} else {
			cwd = wtPath
			log.Printf("[exec] %s: created worktree at %s", executionID, wtPath)
			// Update issue state with worktree path
			if eng != nil {
				if iss, ok := eng.State.Issues[issueID]; ok {
					iss.Worktree = wtPath
					iss.Branch = "feat/" + slug
					iss.Status = engine.IssueImplementing
					eng.SaveIssue(iss)
				}
			}
		}
	...
	go func() {
		cfg := claude.RunConfig{
			...
			Model:        "sonnet",
			SessionID:    sessionID,
			MaxTurns:     maxTurnsForPhase(req.Phase),
		...
		// Post-execution: push branch + create PR for executing phase
		if req.Phase == "executing" && cwd != "." {
			issueID, issueTitle := parseIssueParam(req.Params["issue"])

			// Safety net: commit any uncommitted changes
			if commitErr := gitops.CommitAll(cwd, "feat: implement "+issueID); commitErr != nil {
			...
			if pushErr := gitops.PushBranch(cwd); pushErr != nil {
				...
				s.execMgr.AppendMessage(executionID, engine.Message{
					Role:    "system",
					Content: "Warning: failed to push branch: " + pushErr.Error(),
				})
			} else {
				prURL, exists := gitops.PRExists(cwd)
				if !exists {
					prURL, err = gitops.CreatePR(cwd, issueTitle,
						fmt.Sprintf("Resolves %s\n\nGenerated by Agentic SDLC", issueID), "main")
					if err != nil {
						log.Printf("[exec] %s: create PR failed: %v", executionID, err)
						s.execMgr.AppendMessage(executionID, engine.Message{
							Role:    "system",
							Content: "Warning: failed to create PR: " + err.Error(),
						})
					}
				}
				if prURL != "" {
					log.Printf("[exec] %s: PR created: %s", executionID, prURL)
					s.execMgr.AppendMessage(executionID, engine.Message{
						Role:    "system",
					...
					if eng != nil {
						if iss, ok := eng.State.Issues[issueID]; ok {
							iss.PRURL = prURL
							iss.Status = engine.IssueAwaitingHuman
							eng.SaveIssue(iss)
						}
					}
				}
...
// maxTurnsForPhase returns a turn limit for the Claude session to prevent runaway
// executions and OOM kills. 0 means no limit (used for lightweight phases).
func maxTurnsForPhase(phase string) int {
	switch phase {
	case "executing":
		return 200 // Implementation can be long but must be bounded
...
// Also handles Linear URLs like "https://linear.app/team/issue/IGN-63/some-title".
// If no colon separator is found, the whole string is treated as the ID.
func parseIssueParam(s string) (id, title string) {
	s = strings.TrimSpace(s)

	// Handle Linear URLs: extract identifier and title from the URL path
	if strings.Contains(s, "linear.app") && strings.Contains(s, "/issue/") {
		linearID, linearTitle := extractLinearInfo(s)
		if linearID != "" {
			return linearID, linearTitle
		}
	}
...
// extractLinearInfo extracts the issue identifier and title from a Linear URL.
// e.g., "https://linear.app/yalo/issue/IGN-63/implement-booking-flow" → ("IGN-63", "implement booking flow")
func extractLinearInfo(rawURL string) (id, title string) {
	// Find /issue/ segment
	idx := strings.Index(rawURL, "/issue/")
	if idx < 0 {
		return "", ""
	}
	rest := rawURL[idx+len("/issue/"):] // e.g., "IGN-63/implement-booking-flow"
	parts := strings.SplitN(rest, "/", 2)
	id = parts[0] // e.g., "IGN-63"
	if len(parts) > 1 {
		// Convert slug to readable title: "implement-booking-flow" → "implement booking flow"
		title = strings.ReplaceAll(parts[1], "-", " ")
		// Trim query params if present
		if qi := strings.IndexByte(title, '?'); qi >= 0 {
			title = title[:qi]
		}
...
// serverSlugify converts a string to a URL-safe slug.
func serverSlugify(s string) string {
